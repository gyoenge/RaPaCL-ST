# OMP_NUM_THREADS=4 torchrun --nproc_per_node=2 -m rapacl._radtranstab_mlp 2>&1 | tee log_radtranstab_mlp.log
# nohup bash -c 'OMP_NUM_THREADS=4 torchrun --nproc_per_node=2 -m rapacl._radtranstab_mlp' > log_radtranstab_mlp.log 2>&1 &
...
class RadiomicsTransTabMLPGeneModel(nn.Module):
    ...
    def forward(self, radiomics: torch.Tensor) -> torch.Tensor:
        rad_feat = self.radiomics_encoder(radiomics)
        gene_pred = self.gene_head(rad_feat)
        return gene_pred
...
def train_one_epoch(
    ...
    for batch in loader:
        radiomics, gene = unpack_batch(batch, device)
...
def run_one_fold(
    ...
    for epoch in range(max_epochs):
        ...
        if val_pcc > best_pcc:
            ...
            if is_main_process():
                ckpt_path = os.path.join(save_dir, "best_radtranstab_mlp.pt")
                state_dict = (
...
def main():
    ...
    if is_main_process():
        ...
        print("[INFO] experiment: radtranstab_mlp")
