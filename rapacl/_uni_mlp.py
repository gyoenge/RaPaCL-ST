...
class UNIMLPGeneModel(nn.Module):
    def __init__(self, num_genes: int):
        super().__init__()

        self.pathomics_encoder = FrozenUNIEncoder()
...
def build_uni_mlp_model(
    device: torch.device,
    num_genes: int,
) -> UNIMLPGeneModel:
    model = UNIMLPGeneModel(num_genes=num_genes)
...
def evaluate(
    ...
    if distributed:
        ...
        gathered_preds = [torch.zeros_like(preds) for _ in range(torch.distributed.get_world_size())]
        gathered_targets = [torch.zeros_like(targets) for _ in range(torch.distributed.get_world_size())]
...
def run_one_fold(
    ...
    if is_main_process():
        ...
        print("[INFO] model: Frozen UNI + MLP gene head")

    model = build_uni_mlp_model(
        device=device,
        num_genes=num_genes,
    )
    ...
    max_epochs = train.STAGE2_EPOCHS if hasattr(train, "STAGE2_EPOCHS") else train.MAX_EPOCHS
    ...
    for epoch in range(max_epochs):
        ...
        if val_pcc > best_pcc:
            ...
            if is_main_process():
                ckpt_path = os.path.join(save_dir, "best_uni_mlp.pt")
...
def main():
    ...
    if is_main_process():
        ...
        print("[INFO] experiment: uni_frozen_mlp")
